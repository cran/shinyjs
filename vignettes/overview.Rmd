...
A typical Shiny app has a UI portion and a server portion. As mentioned above, `useShinyjs()` must be called in the Shiny app's UI, and it's best to include it near the top as a convention. For example, here is a minimal Shiny app that uses `shinyjs`:
...
This is how most Shiny apps should initialize `shinyjs`, but there are a few common scenarios that should be treated a little differently: using `shinyjs` in interactive documents, using `shinyjs` in Shiny dashboards, and using `shinyjs` in Shiny apps that use a `navbarPage` layout.

### Using shinyjs in interactive R Markdown documents

It is possible to embed Shiny components in an R Markdown document, resulting in interactive R Markdown documents. Interactive documents don't explicitly specify a UI and a server, but using `shinyjs` is still easy; simply call `useShinyjs(rmd = TRUE)` (note the `rmd = TRUE` argument). For example, the following code can be used inside an R Markdown code chunk:
